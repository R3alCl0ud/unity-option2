...
namespace Level
{
	public class LevelControl : MonoBehaviour
	{
		...
		void Update ()
		{
			if (startTimer > 0 && currentEnemies == maxEnemies && enemies.Length == 0)
			{          //if we're either counting down or still waiting
				if (Called == false)
				{
					roundEnd ();
				}				
			...
			if (roundIsOver == true)
			{                                            //if the round is in over state
				if (Input.GetKeyDown ("space"))
				{                   //"press space to start the countdown timer
					if (startTimer > 0)
					{                        // but only if its not 0
